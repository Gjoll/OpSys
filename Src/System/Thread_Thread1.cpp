...
/*
  $Revision: 1.1 $
  $Modtime:   01 May 1993 09:32:14  $
  $Workfile:   thread1.cpp  $
*/
...
Parameters:
   t                   The address of the function to execute as a 
                       thread.
   d                   A pointer to an arbitrary data object that 
                       is passed to the function *iTPtr.
...
Thread::Thread(TPtr iTPtr, void* iData, unsigned int iStackLen,
		short iPriority, const char* name) :
	STask(iStackLen, iPriority, name),
	data(iData), tPtr(iTPtr)
{
  objType = ThreadType;
  start();
}
